...
class SubprocVecEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        return _flatten_obs(obs, self.observation_space), np.vstack(rews), np.vstack(dones), infos
...
def _flatten_obs(obs, space):
    ...
    if isinstance(space, gym.spaces.Dict):
        ...
        fo = []
        for k, box in space.spaces.items():
            if box.shape == (1,):
                b = (k, np.vstack([o[k] for o in obs]))
            else:
                b = (k, np.stack([o[k] for o in obs]))
            fo.append(b)
        return OrderedDict(fo)
        ...
        )  # TODO: vstack over stack for (1,) box spaces
    else:
        return np.stack(obs)
