...
class VecEnv(ABC):
    ...
    def __getattr__(self, attr):
        return self.get_attr(attr, indices=0)[0]
    ...
    def _get_seeds(self, base_seed, n):
        seed_sequence = np.random.SeedSequence(entropy=base_seed)
        return [int(x) for x in seed_sequence.generate_state(n)]
